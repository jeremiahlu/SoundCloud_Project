...
const validateAlbumEdit = [
  check("title").exists().notEmpty().withMessage("Album title is required"),
  handleValidationErrors,
];
...
router.get("/:albumId", async (req, res, next) => {
  ...
  // const albumObject = album.toJSON();
  // console.log(albumObject, "OBJ$!@#@$!");
  // albumObject.artist = albumObject.Artist;
  // albumObject.songs = albumObject.Songs;

  // delete albumObject.Artist;
  // delete albumObject.Songs;
  // console.log("ALBUMM!$@@$$", typeof album);
  if (!album || album === null) {
...
router.post(
  "/",
  [validateAlbumEdit, validateCreation],
  async (req, res, next) => {
...
router.patch(
  "/:albumId",
  [validateAlbumEdit, requireAuth, restoreUser],
  async (req, res, next) => {
    const { albumId } = req.params;
    const { title, description, previewImage } = req.body;
